	...
	REAL, PARAMETER :: DT = 0.001
  	...
  	OPEN(UNIT=10, FILE = 'MAGNUS_51MM.out')
...
!INITIALIZE THE BALL
	...
	!WRITE(10,*) ' - ANGLE: ', ANGLE*57.3
	  ...
	  !WRITE(10,*) ' - '
		...
		BALL% V = SQRT(BALL%V2)
		G = -9.8 + 1868 * RAD * RAD * RAD
		CD = -1401 * RAD * RAD
		MAG = 254.71 * RAD * RAD * BALL(5)%V2 * (BALL(5)%V)/G
		!PRINT*, 'V: ', BALL%V

  		BALL% AX = CD * BALL% V * BALL% VX
  		!BALL% AY = CD * BALL% V * BALL% VY
  		BALL% AZ = G + MAG + CD * BALL% V * BALL% VZ
		...
		BALL% AZMID = G + MAG + CD * BALL% VMID * BALL% VMID
